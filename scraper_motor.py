...
def execute_scraper(start_date_str, end_date_str):
    ...
    while current <= end:
        ...
        import liturgia_local_extractor as local_ext
        data_es = local_ext.extract_misa_local(date_key)
        if not data_es:
            print(f"[CASCADA INFO] No hallado en Local, ejecutando Fallback a Scraper Web (CEM)...")
        ...
        horas_local = local_ext.extract_horas_local(date_key)
        
        laudes_data = horas_local["laudes"] if (horas_local and horas_local.get("laudes")) else extract_horas_data(current, "laudes")
        if laudes_data:
            db[date_key]['laudes'] = laudes_data
            print("✅ Laudes extraídas y acopladas.")
        
        visperas_data = horas_local["visperas"] if (horas_local and horas_local.get("visperas")) else extract_horas_data(current, "visperas")
        if visperas_data:
    ...
    # ── Parche de metadatos canónicos (sin API) ──────────────────────────────
    # Aplica título, color, grado y esqueleto desde el calendario litúrgico
    # hardcodeado. Solo rellena campos vacíos; nunca sobreescribe datos del ETL.
    try:
        import patch_metadatos_ordo as _ordo
        _ordo.patch_db()
        # Recargar DB tras el parche
        with open(db_path, 'r', encoding='utf-8') as _f:
            _c = _f.read()
        import re as _re
        db = json.loads(_re.sub(r'^window\.liturgiaData\s*=\s*', '', _c.strip()).rstrip(';'))
        print("✅ Metadatos canónicos aplicados desde ordo.")
    except Exception as _e:
        print(f"⚠️ Parche de metadatos falló: {_e}")
    ...
    try:
        ...
        for d in db.keys():
            ...
            day_json = json.dumps({d: db[d]}, ensure_ascii=False)
...
<body>
    ...
    <script>
        ...
        window.addEventListener('DOMContentLoaded', () => {{
            ...
            with open(f"liturgia_{d}.html", "w", encoding="utf-8") as html_file:
                html_file.write(html_content)
        print("✅ HTML Standalone generados con éxito.")
